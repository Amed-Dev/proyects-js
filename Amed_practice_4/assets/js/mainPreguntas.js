...
async function showTrivia() {
  try {
    const urlData = "./assets/data/preguntas.json";
...
function showTopic(data) {
  let contentTopics = document.createDocumentFragment();
  data.forEach((topic, i) => {
    let card = document.createElement("div");
    card.className = "card w-md-18";
    card.innerHTML = `
    ...
    modal.tabIndex = -1;
    modal.ariaLabelledBy = "staticBackdropLabel";
    ...
    `;
    contentTopics.appendChild(card);
    contentTopics.appendChild(modal);
  });

  $("#trivia-board").appendChild(contentTopics);

  $$("#btn-start").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      let i = e.target.getAttribute("data-quizz-number");
...
function showQuestions(data, startTime, i) {
  ...
  let triviaContainer = $("#trivia_container");
  let contentQuestions = document.createDocumentFragment();

  data[i].preguntas.forEach((q) => {
    let questionsField = document.createElement("ul");
    questionsField.className = "list-group my-3 questionsField";
    questionsField.dataset.questionNumber = q.numero;
    questionsField.dataset.correctAnswer = q.correcta;
    questionsField.dataset.score = scorePoints;
    ...
    `;
    contentQuestions.appendChild(questionsField);
  });

  triviaContainer.appendChild(contentQuestions);
...
function updateAcuraccyBar() {
  ...
  if (progress === 0) {
    ...
    progressLabel.style.left = `calc(${progress}% - 45px)`;
  } else {
